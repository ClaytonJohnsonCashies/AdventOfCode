...
void Main()
{
	...
	{
		//var digits = Part1(line);
		var digits = Part2(line);
		
		var firstDigit = digits.First();
		var lastDigit = digits.Last();
		var output = int.Parse(string.Join("", firstDigit, lastDigit));
		(output + " => " + line + " => " + string.Join(",", digits)).Dump();
		values.Add(output);
	}
	
	var result = values.Sum().Dump();
}

int[] Part1(string line)
{
	return line.Where(c => c >= 48 && c <= 57)
...
int[] Part2(string line)
{
	...
	foreach (char c in chars)
	{
		...
		word4 += c;
		word5 += c;
		for (int ii = 0; ii < digitWords.Length; ii++)
		{
			if (word3.Contains(digitWords[ii]) && digitWords[ii].Length == 3)
			{
				digits.Add(ii);
			}
			if (word4.Contains(digitWords[ii]) && digitWords[ii].Length == 4)
			{
				digits.Add(ii);
			}
			if (word5.Contains(digitWords[ii]) && digitWords[ii].Length == 5)
			{
				digits.Add(ii);
			}
		}
...
public class Inputs
{
	public const string Test = "41onevfsgvssxnpsix38four";
	
	public const string Example1 = @"1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet";
...
7pqrstsixteen";
	...
	public const string Input1 = @"9sixsevenz3
