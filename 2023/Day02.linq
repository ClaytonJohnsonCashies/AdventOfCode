...
void Main()
{
	...
	foreach (var line in lines)
	{
		...
		var game = new Game() { Id = int.Parse(pairs[1]) };
		for (int ii = 3; ii < pairs.Length; ii += 3)
		{
			if (pairs[ii + 1] == "red")
			{
				game.Red = Math.Max(game.Red, int.Parse(pairs[ii]));
			}
			else if (pairs[ii + 1] == "green")
			{
				game.Green = Math.Max(game.Green, int.Parse(pairs[ii]));
			}
			else if (pairs[ii + 1] == "blue")
			{
				game.Blue = Math.Max(game.Blue, int.Parse(pairs[ii]));
			}
		}
		games.Add(game);
	}
	
	var possibleGames = games.Where(g => g.PassPart1).Select(g => g.Id).Dump();
	var sumIds = possibleGames.Sum().Dump("Sum IDs");
	
	var sumPowerSetOfCubes = games.Sum(g => g.PowerSetOfCubes).Dump("Part 2");
}
...
public class Game
{
	...
	public bool PassPart1 => (Red <= 12) && (Green <= 13) && (Blue <= 14);
	public long PowerSetOfCubes => Red * Green * Blue;
}
