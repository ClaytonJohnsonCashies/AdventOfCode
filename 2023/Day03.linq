...
void Main()
{
	...
	for (int row = 0; row < lines.Length; row++)
	{
		...
		for (int column = 0; column < line.Length; column++)
		{
			...
			if (numberStr.Length > 0)
			{
				numbers.Add(new Number() 
				{
					...
					Position = (row, column - numberStr.Length),
		...
		if (numberStr.Length > 0)
		{
			...
			{
				Value = int.Parse(numberStr),
				Position = (row, line.Length - numberStr.Length),
				Length = numberStr.Length
			});
		}
	}
	...
	var unOverlappingNumbers = numbers.Where(n => !symbols.Any(s => Overlaps(n, s)));
	var overlappingNumbers = numbers.Where(n => symbols.Any(s => Overlaps(n, s)));
	var overlappingSum = overlappingNumbers.Sum(n => n.Value).DumpTell();
	
	foreach (var symbol in symbols)
	{
		foreach (var number in numbers.Where(n => Overlaps(n, symbol)))
		{
			symbol.AddAdjacentNumber(number);
		}
	}
	
	var gears = symbols.Where(s => s.IsGear);
	var sumGearRatio = gears.Sum(g => g.GearRatio).Dump();
}

public bool Overlaps(Number number, Symbol symbol)
{
	if ((number.Position.Row >= symbol.Position.Row - 1) && (number.Position.Row <= symbol.Position.Row + 1))
	{
		// within row range
		if (number.Position.Column <= symbol.Position.Column + 1 && symbol.Position.Column - 1 <= number.Position.Column + number.Length - 1)
		{
			// within column range
			return true;
		}
...
public class Symbol
{
	...
	private List<Number> _adjacentNumbers = new();
	public bool IsGear => Value == '*' && _adjacentNumbers.Count() == 2;
	public long GearRatio => _adjacentNumbers.Count() == 2 ? _adjacentNumbers[0].Value * _adjacentNumbers[1].Value : 0;
	
	public void AddAdjacentNumber(Number number) => _adjacentNumbers.Add(number);
}
