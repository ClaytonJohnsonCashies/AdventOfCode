...
void Main()
{
	...
	var seeds = lines[0].Substring(7).Split(' ').Select(r => UInt32.Parse(r)).ToArray();
	//seeds.Dump();

	var seedToSoilStart = Array.IndexOf(lines, "seed-to-soil map:").Dump();
	var soilToFertilizerStart = Array.IndexOf(lines, "soil-to-fertilizer map:").Dump();
	var fertilizerToWaterStart = Array.IndexOf(lines, "fertilizer-to-water map:").Dump();
	var waterToLightStart = Array.IndexOf(lines, "water-to-light map:").Dump();
	var lightToTemperatureStart = Array.IndexOf(lines, "light-to-temperature map:").Dump();
	var temperatureToHumidityStart = Array.IndexOf(lines, "temperature-to-humidity map:").Dump();
	var humidityToLocationStart = Array.IndexOf(lines, "humidity-to-location map:").Dump();

	var mappings = new Mappings()
	{
		SeedToSoilMapping               = CreateMapping(lines.Skip(seedToSoilStart+1).Take(soilToFertilizerStart - seedToSoilStart - 2).ToArray()),
		SoilToFertilizerMapping 		= CreateMapping(lines.Skip(soilToFertilizerStart + 1).Take(fertilizerToWaterStart - soilToFertilizerStart - 2).ToArray()),
		FertilizerToWaterMapping 		= CreateMapping(lines.Skip(fertilizerToWaterStart + 1).Take(waterToLightStart - fertilizerToWaterStart - 2).ToArray()),
		WaterToLightMapping 			= CreateMapping(lines.Skip(waterToLightStart + 1).Take(lightToTemperatureStart - waterToLightStart - 2).ToArray()),
		LightToTemperatureMapping 		= CreateMapping(lines.Skip(lightToTemperatureStart + 1).Take(temperatureToHumidityStart - lightToTemperatureStart - 2).ToArray()),
		TemperatureToHumidityMapping 	= CreateMapping(lines.Skip(temperatureToHumidityStart + 1).Take(humidityToLocationStart - temperatureToHumidityStart - 2).ToArray()),
		HumidityToLocationMapping 		= CreateMapping(lines.Skip(humidityToLocationStart + 1).ToArray())
	};
	
	foreach (var seed in seeds)
	{
		$"Seed number {seed} corresponds to soil number {mappings.FindMappingOutput(mappings.SeedToSoilMapping, seed)}".Dump();
	}
	
	// Part 1
	var locations = seeds.Select(s => mappings.GetLocationNeedingSeed(s)).ToArray().Dump();
	var lowestLocation = locations.Min(x => x).DumpTell();
	
	// Part 2
	var seedRanges = new List<(long Start, long Length)>();
	for (int ii = 0; ii < seeds.Length; ii += 2)
	{
		seedRanges.Add((seeds[ii], seeds[ii+1]));
	}
	
	var rangeLocations = seedRanges.Select(s => mappings.GetMinimumLocationNeedingSeeds(s.Start, s.Length)).ToArray();
	var rangeLowestLocation = rangeLocations.Min(x => x).DumpTell();
}

public class Mappings
{
	public Mapping[] SeedToSoilMapping { get; set; }
	public Mapping[] SoilToFertilizerMapping { get; set; }
	public Mapping[] FertilizerToWaterMapping { get; set; }
	public Mapping[] WaterToLightMapping { get; set; }
	public Mapping[] LightToTemperatureMapping { get; set; }
	public Mapping[] TemperatureToHumidityMapping { get; set; }
	public Mapping[] HumidityToLocationMapping { get; set; }
	
	public long FindMappingOutput(Mapping[] mapping, long value)
	{
		var outputResult = mapping.Select(m => m.GetOutput(value)).SingleOrDefault(m => m.HasValue);
		return outputResult ?? value;
	}

	public (long Start, long Length)[] FindMappingOutput(Mapping[] mapping, long start, long length)
	{
		var overlapping = mapping.Where(m => m.DoesOverlap(start, length)).ToArray();
		if (overlapping.Count() == 0)
			return [(start, length)];
			
		var partitions = new List<(long Start, long Length)>();
		var remaining = (start, length);
		foreach (var overlap in overlapping)
		{
			if (remaining.start < overlap.Source)
			{
				var newLength = overlap.Source - remaining.start;
				partitions.Add((remaining.start, newLength));
				remaining.start = overlap.Source;
				remaining.length -= newLength;
			}
			if (remaining.start + remaining.length <= overlap.Source + overlap.Length)
			{
				partitions.Add((overlap.Destination + (remaining.start - overlap.Source), remaining.length));
				remaining.length = 0;
				break;
			}
			
			partitions.Add((overlap.Destination + (remaining.start - overlap.Source), overlap.Length - (remaining.start - overlap.Source)));
			
			var endLength = (remaining.start + remaining.length - 1) - (overlap.Source + overlap.Length - 1);
			remaining.start = overlap.Source + overlap.Length;
			remaining.length = endLength;
		}
		
		if (remaining.length > 0)
		{
			partitions.Add((remaining.start, remaining.length));
		}
		
		// this needs to be converted to destination
		return partitions.ToArray();
	}

	public long GetLocationNeedingSeed(long seed)
	{
		var soil = FindMappingOutput(SeedToSoilMapping, seed);
		var fertilizer = FindMappingOutput(SoilToFertilizerMapping, soil);
		var water = FindMappingOutput(FertilizerToWaterMapping, fertilizer);
		var light = FindMappingOutput(WaterToLightMapping, water);
		var temperature = FindMappingOutput(LightToTemperatureMapping, light);
		var humidity = FindMappingOutput(TemperatureToHumidityMapping, temperature);
		var location = FindMappingOutput(HumidityToLocationMapping, humidity);

		//$"Seed {seed}, soil {soil}, fertilizer {fertilizer}, water {water}, light {light}, temperature {temperature}, humidity {humidity}, location {location}".Dump();
		return location;
	}
	
	public long GetMinimumLocationNeedingSeeds(long start, long length)
	{
		var soils 			= FindMappingOutput(SeedToSoilMapping, start, length);
		var fertilizers 	= soils.SelectMany(s => FindMappingOutput(SoilToFertilizerMapping, s.Start, s.Length)).ToArray();
		var waters 			= fertilizers.SelectMany(s => FindMappingOutput(FertilizerToWaterMapping, s.Start, s.Length)).ToArray();
		var lights 			= waters.SelectMany(s => FindMappingOutput(WaterToLightMapping, s.Start, s.Length)).ToArray();
		var temperatures 	= lights.SelectMany(s => FindMappingOutput(LightToTemperatureMapping, s.Start, s.Length)).ToArray();
		var humiditys 		= temperatures.SelectMany(s => FindMappingOutput(TemperatureToHumidityMapping, s.Start, s.Length)).ToArray();
		var locations 		= humiditys.SelectMany(s => FindMappingOutput(HumidityToLocationMapping, s.Start, s.Length)).ToArray();
		
		var minLocation = locations.Min(x => x.Start);
		minLocation.Dump();
		return minLocation;
	}
}
...
public class Mapping
{
	...
	public bool DoesOverlap(long start, long length)
	{
		// (x1 <= y2) && (y1 <= x2)
		return Source <= start + length && start <= Source + Length - 1;
	}
}

// <sourceid, destinationid>
public Mapping[] CreateMapping(string[] mappingRules)
{
	...
	foreach (var rule in mappingRules)
	{
		...
		var values = rule.Split(' ').Select(r => long.Parse(r)).ToArray();
	...
	//mapping.Dump();
	return mapping.OrderBy(m => m.Source).ToArray();
}
