...
void Main()
{
	...
	var DFS = new TreeSearcher<Path>(
		state => state.Steps > 0 && map[state.CurrentPosition] == 'S',
		currState =>
		{
			switch (map[currState.CurrentPosition])
			{
				case '|':
					if (currState.PreviousPosition.Y - currState.CurrentPosition.Y > 0)
						return currState.CurrentPosition.Y > 0 ? [currState.Up()] : [];
					else
						return currState.CurrentPosition.Y < height ? [currState.Down()] : [];
				case '-':
					if (currState.PreviousPosition.X - currState.CurrentPosition.X > 0)
						return currState.CurrentPosition.X > 0 ? [currState.Left()] : [];
					else
						return currState.CurrentPosition.X < width ? [currState.Right()] : [];
				case 'L':
					if (currState.PreviousPosition.X == currState.CurrentPosition.X)
						return currState.CurrentPosition.X < width ? [currState.Right()] : [];
					else
						return currState.CurrentPosition.Y > 0 ? [currState.Up()] : [];
				case 'J':
				...
				case 'S':
					...
					if (currState.CurrentPosition.Y > 0 		&& new[] { '|', '7', 'F'}.Contains(map[currState.UpPosition]))
						options.Add(currState.Up());
					if (currState.CurrentPosition.Y < height 	&& new[] { '|', 'L', 'J' }.Contains(map[currState.DownPosition]))
						options.Add(currState.Down());
					if (currState.CurrentPosition.X > 0 		&& new[] { '-', 'L', 'F' }.Contains(map[currState.LeftPosition]))
						options.Add(currState.Left());
					if (currState.CurrentPosition.X < width 	&& new[] { '-', '7', 'J' }.Contains(map[currState.RightPosition]))
						options.Add(currState.Right());
	...
	var solution = DFS.DepthFirstSearch(new Path(0, startingPosition, startingPosition, new List<(int, int)>()));
	//solution.DumpTell();
	(solution.Steps / 2).Dump("Halve Steps");
	
	
	// Perform point-in-polygon check https://en.wikipedia.org/wiki/Point_in_polygon
	int pointsInPolygon = 0;
	var polygonPoints = solution.PositionHistory.Select(x => ((float)x.Item1, (float)x.Item2)).ToArray();
	var displayMap = new Dictionary<(int, int), char>();
	
	foreach (var point in map.Keys)
	{
		if (polygonPoints.Contains(point))
		{
			displayMap.Add(point, map[point]);
			continue;
		}
		
		if (IsPointInPolygon4(polygonPoints, point))
		{
			displayMap.Add(point, 'I');
			pointsInPolygon++;
		}
		else
		{
			displayMap.Add(point, 'O');
		}
		
//		if (point.Item1 == 13 && point.Item2 == 6)
//		{
//			"test".Dump();
//		}
//		
//		var intersectingPointsLeft = 0;
//		var intersectingPointsRight = 0;
//		if (point.Item1 > 0 && point.Item1 < width)
//		{
//			for (int x = point.Item1; x < width + 5; x++)
//			{
//				if (polygonPoints.Contains((x, point.Item2)) && (map.ContainsKey((x, point.Item2)) && map[(x, point.Item2)] != '-'))
//					intersectingPointsLeft++;
//			}
//			for (int x = point.Item1; x > -5; x--)
//			{
//				if (polygonPoints.Contains((x, point.Item2)) && (map.ContainsKey((x, point.Item2)) && map[(x, point.Item2)] != '-'))
//					intersectingPointsRight++;
//			}
//		}
//
//		if ((intersectingPointsLeft == 0 || intersectingPointsRight == 0) || (intersectingPointsLeft > 0 && intersectingPointsLeft % 2 == 0))// &&
//			//(intersectingPointsRight > 0 && intersectingPointsRight % 2 == 0))
//		{
//			
//			displayMap.Add(point, 'O');
//		}
...
//		}
	...
	pointsInPolygon.DumpTell();
	RenderMap(displayMap, width, height);
}

public void RenderMap(Dictionary<(int, int), char> map, int width, int height)
{
...
public static bool IsPointInPolygon4((float X, float Y)[] polygon, (float X, float Y) testPoint)
{
	bool result = false;
...
public class Path
{
	...
	public List<(int, int)> PositionHistory { get; set; }
	
	public Path (int steps, (int X, int Y) currentPosition, (int X, int Y) previousPosition, List<(int, int)> history)
	{
		Steps = steps;
		CurrentPosition = currentPosition;
		PreviousPosition = previousPosition;
		
		PositionHistory = new List<(int, int)>(history);
		PositionHistory.Add(CurrentPosition);
	}
	
	public Path Up()
	{
		return new Path(Steps + 1, (CurrentPosition.X, CurrentPosition.Y - 1), CurrentPosition, PositionHistory);
	}
	...
	public Path Left()
	{
		return new Path(Steps + 1, (CurrentPosition.X - 1, CurrentPosition.Y), CurrentPosition, PositionHistory);
	}
	...
	public (int, int) UpPosition => (CurrentPosition.X, CurrentPosition.Y - 1);
	public (int, int) DownPosition => (CurrentPosition.X, CurrentPosition.Y + 1);
	public (int, int) LeftPosition => (CurrentPosition.X - 1, CurrentPosition.Y);
	public (int, int) RightPosition => (CurrentPosition.X + 1, CurrentPosition.Y);
}
